...
uint8_t SX1272::ON()
{
 ...
 //     SPI.setClockDivider(SPI_CLOCK_DIV4);
    ...
    if (version == 0x22) { // AGGE kan vi läsa ut om 72,73 ? Gäller Version code of the chip. Bits 7-4 give the full revision number. bits 3-0 give the metal mask revision number
        // sx1272
        Serial.println(F("SX1272 detected, starting"));
        _board = SX1272Chip;
    } else {
        // sx1276? // AGGE kan vi läsa ut om 76,77,78 ?? //Version code of the chip. Bits 7-4 give the full revision number. bits 3-0 give the metal mask revision number
        digitalWrite(SX1272_RST, LOW);
        delay(100);
        digitalWrite(SX1272_RST, HIGH);
        delay(100);
...
/*!  --------------------------------------------- Not Fixed yet ------------------------------------------------------------------
 ...
 Returns: Integer that determines if there has been any error
   ...
   state = -1 --> Forbidden command for this protocol
 Parameters:
   fq: frequency channel value to set in configuration.
    = FQ_in_Hz*RH_LORA_FCONVERT, RH_LORA_FCONVERT=(524288 / RH_LORA_FXOSC), RH_LORA_FXOSC=32000000.0
*/
int8_t SX1272::setFrequency(uint32_t fq)
{
    ...
    uint32_t freqConv;
        ...
        //! Calculate register setting from frequency

        freqConv = fq*RH_LORA_FCONVERT; // rounding to correct value has to be done

#if (SX1272_debug_mode > 1)
    Serial.println();
    Serial.println(F("Starting 'setFrequency'"));
    Serial.println(freqConv);
#endif
    ...
    freq3 = ((freqConv >> 16) & 0x0FF);		// frequency channel MSB
    freq2 = ((freqConv >> 8) & 0x0FF);		// frequency channel MIB
    freq1 = (freqConv & 0xFF);				// frequency channel LSB
    ...
    if(!isChannel(freqConv) )
    {
        state = -1;
#if (SX1272_debug_mode > 1)
        Serial.print(F("** Frequency channel "));
        Serial.print(freqConv, HEX);
        Serial.println(F("is not a correct value **"));
        Serial.println();
#endif
    }
...
/*! added by Agge
 Function: Sets the module in Sleep mode.
 Returns: Integer that determines if there has been any error
   state = 2  --> The command has not been executed
   state = 1  --> There has been an error while executing the command
   state = 0  --> The command has been executed with no errors
